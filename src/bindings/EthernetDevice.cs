...
namespace TAPCfg {
	public class EthernetDevice : IDisposable {
		...
		[DllImport("libtapcfg")]
		private static extern int tapcfg_iface_set_ipv4(IntPtr tapcfg, IntPtr addr, byte[] netbits);
		[DllImport("libtapcfg")]
		private static extern int tapcfg_iface_set_ipv6(IntPtr tapcfg, IntPtr addr, byte[] netbits);
	}
}
